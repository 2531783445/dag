...
func S_File(store KVStore, node File, h hash.Hash) ([]byte, []byte) {
	t := []byte("blob")
...
func S_Dir(store KVStore, dir Dir, h hash.Hash) ([]byte, []byte) {
	t := []byte("tree")
	tree := Object{
		Links: make([]Link, 0),
		Data:  make([][]byte, 0),
	...
	for it.Next() {
		...
		case FILE:
			file := node.(File)
			fileHash, _ := S_File(store, file, h)
		...
		case DIR:
			subDir := node.(Dir)
			subDirHash, _ := S_Dir(store, subDir, h)
			tree.Links = append(tree.Links, Link{
				Name: subDir.Name(),
				Hash: subDirHash,
...
func Add(store KVStore, node Node, h hash.Hash) []byte {
	...
	case FILE:
		file := node.(File)
		fileHash, data := S_File(store, file, h)
	...
	case DIR:
		...
		dirHash, data := S_Dir(store, dir, h)
		obj = Stack[len(Stack)-1]
